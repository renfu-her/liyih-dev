...
$link_B   		=	$_POST["link"];
$banner2_href    =	$_POST["banner2_href"];
...
{//如果有上傳檔案
	...
	//圖片上傳
		...
		$pic_b_del=$rows["banner2_pic_b"];
		$pic_s_del=$rows["banner2_pic_s"];
	...
	//查詢圖檔		
		...
		$result->execute();
		$rows = $result->fetch(PDO::FETCH_ASSOC);
		$pic_b=$rows["banner2_pic_b"];
		$pic_s=$rows["banner2_pic_s"];
	}
...
{//如果有上傳檔案
	...
	//圖片上傳
		...
		$result->execute();
		$rows = $result->fetch(PDO::FETCH_ASSOC);
		$pic_b_mob_del=$rows["banner2_pic_b_mob"];
		$pic_s_mob_del=$rows["banner2_pic_s_mob"];
	...
	//查詢圖檔		
		...
		$result->execute();
		$rows = $result->fetch(PDO::FETCH_ASSOC);
		$pic_b_mob=$rows["banner2_pic_b_mob"];
		$pic_s_mob=$rows["banner2_pic_s_mob"];
	}


$sql="UPDATE `banner2` SET 
`banner2_hide` =:hide,
`banner2_href` =:banner2_href,
`banner2_title` =:banner2_title,
`banner2_link` =:link_B,
`banner2_pic_b` =:pic_b,
`banner2_pic_s` =:pic_s,
`banner2_pic_b_mob` =:pic_b_mob,
`banner2_pic_s_mob` =:pic_s_mob,
`banner2_date` =:edittime
WHERE `banner2_no` =:edit_no ;";

$result = $db->prepare("$sql");//防sql注入攻擊
	// 數值PDO::PARAM_INT  字串PDO::PARAM_STR
	$result->bindValue(':link_B', $link_B, PDO::PARAM_STR);
	$result->bindValue(':banner2_href', $banner2_href, PDO::PARAM_STR);
	$result->bindValue(':banner2_title', $banner2_title, PDO::PARAM_STR);
